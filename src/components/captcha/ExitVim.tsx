...
interface ExitVimProps {
  setL: (paragraph: string) => void;
  setP: (paragraph: string) => void;
  setSuccess: (val: boolean) => void;
}

export const ExitVim = ({ setL, setP, setSuccess }: ExitVimProps) => {
  ...
  if (closed) {
    lines = [
      `Exited vim           `,
      `captcha@iamnotarobot.us: BLUE~END $                                                      `,
    ];
  }

  return (
    <div className="h-full w-[850px] bg-black font-mono relative">
      {/* Cursor */}
      {!inputting && !closed && (
